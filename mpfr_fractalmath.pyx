...
def mandelbrot_steps(s, radius, max_iter):                                     
    ...
    start_real_py_str = s.real.str(radius=False, more=True).encode('ascii')
    start_imag_py_str = s.imag.str(radius=False, more=True).encode('ascii')
    ...
    mpfr_mandelbrot_steps(&answer, &last_z_real_str, &last_z_imag_str, start_real_str, start_imag_str, float(radius), max_iter, flint.ctx.prec)
